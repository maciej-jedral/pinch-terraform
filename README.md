...
- Providers (`hashicorp/aws`, `kislerdm/neon`) are plugins that translate resources into API calls. `.terraform.lock.hcl` pins their exact versions - commit it.
...
```sh
cp .env.example .env                          # NEON_API_KEY=...
cp terraform.tfvars.example terraform.tfvars  # ssh_public_key, budget_email, neon_org_id
...
~ $7/mo instance + ~ $3.65/mo public IPv4 + ~ $0.80/mo disk ≈ **$12/mo of credits**; Neon and Budgets are $0. The Free plan cannot bill the card; it closes at 6 months or when credits run out. Everything here is rebuilt with one `apply` on the next account - only the state bucket needs migrating (`terraform init -migrate-state`).
